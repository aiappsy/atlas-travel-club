...
- **WhatsApp Cloud API**: Configure Meta Developer App with WhatsApp Product and point webhook to \`/api/concierge/whatsapp\`.`,
  ...
  // ==========================================
  // 2. JUNIOR SOFTWARE ENGINEER MANUAL
  // ==========================================
  {
...
Ensure all production environment variables are configured in the deployment dashboard (\`GEMINI_API_KEY\`, \`STRIPE_SECRET_KEY\`, \`TELEGRAM_BOT_TOKEN\`, etc.).`,
  ...
  // ==========================================
  // 3. GENERAL APP & PLATFORM OPERATIONS MANAGER MANUAL
  // ==========================================
  {
...
- Instruct the front desk agent to search by the **Bedbank Confirmation Reference Number** listed on the B2B Wholesale Voucher.`,
  ...
  // ==========================================
  // 4. CHIEF MARKETING OFFICER & GROWTH LEAD MANUAL
  // ==========================================
  {
...
const fileContent = `export interface ManualChapter {
  id: string;
  title: string;
...
export const ATLAS_TRAINING_MANUALS: TrainingManual[] = ${JSON.stringify(manuals, null, 2)};
`;

fs.writeFileSync(path.join(__dirname, '../src/lib/manualsData.ts'), fileContent, 'utf8');
console.log('Successfully written src/lib/manualsData.ts');
