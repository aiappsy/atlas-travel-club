...
import { Sparkles, TrendingDown, ArrowRight, Building2, CheckCircle2, ShieldCheck, HelpCircle } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-6 space-y-4">
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 sm:p-7 border border-white/20 shadow-2xl space-y-5">
              ...
              {/* Action Button */}
              <Link
                href={`/hotels/${dest.id === 'vegas' ? 'bellagio-vegas' : dest.id === 'cancun' ? 'paradise-cancun' : dest.id === 'paris' ? 'ritz-paris' : dest.id === 'orlando' ? 'orlando-resort-spa' : 'shinjuku-tower-tokyo'}`}
                className="w-full py-3.5 px-6 rounded-2xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-slate-950 font-black text-sm shadow-xl shadow-amber-400/20 transition-all flex items-center justify-center gap-2 group"
              >
                <span>Book This Wholesale Deal</span>
